			...
			unsigned int buffertime, unsigned int periodtime)
{
	...
	if (err < 0) {
		dbg("Access type not available"); //#define printf dbg
		return err;
	}
...
static void * record_thread_proc(void * para)
{
	...
	while(1) {
		...
		if (pcm_read(rec, frames) != static_cast<int>(frames)) {
			return NULL;
		}
		...
		record_dev_id dev, WAVEFORMATEX * fmt)
{
	...
	err = snd_pcm_open((snd_pcm_t **)&rec->wavein_hdl, dev.u.name,  //snd_pcm_open打开PCM设备，是PCM设备硬件留下的接口，其snd_pcm_t为其数据结构类型,具体明细请百度
			SND_PCM_STREAM_CAPTURE, 0);  // SND_PCM_STREAM_CAPTURE捕获
	if(err < 0)
		goto fail;
...
static int get_pcm_device_cnt(snd_pcm_stream_t stream)
{
	...
	if (snd_device_name_hint(-1, "pcm", &hints) < 0)
		return 0;
	n = hints;
	filter = stream == SND_PCM_STREAM_CAPTURE ? const_cast<char *>("Input") : const_cast<char *>("Output");
	while (*n != NULL) {
						...
						char ** desc_out)
{
	...
	*name_out = static_cast<char *>(calloc(sizeof(char *) , (1+cnt)));
	if (*name_out == NULL)
		goto fail;
	*desc_out = static_cast<char *>(calloc(sizeof(char *) , (1 + cnt)));
	if (*desc_out == NULL)
		goto fail;
	...
	while (*n != NULL && i < cnt) {
		...
		*descr = snd_device_name_get_hint(*n, "DESC");
		...
		if (name == NULL || 
			...
			if (*descr == NULL) {
				*descr = static_cast<char *>(malloc(4));
				memset(*descr, 0, 4);
			}
			name++;
			descr++;
...
{
	record_dev_id id; 
	id.u.name = const_cast<char *>("default");
	return id;
}
